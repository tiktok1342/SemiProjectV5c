...
import yang.spring.mvc.vo.GallerVO;
...
public class GallerDAO {
    ...
    @Value("#{jdbc['insertGallerSQL']}")private String insertGallerSQL;
    @Value("#{jdbc['selectGallerSQL']}")private String selectGallerSQL;
    @Value("#{jdbc['selectOneGallerSQL']}")private String selectOneGallerSQL;
    ...
    public boolean insertGaller( GallerVO g ) {
        // sql 정의
        //String sql = "insert into board (title,userid,contents) values (?,?,?)";

        // 매개변수 정의
        Object[] params = new Object[] {
             g.getTitle(), g.getUserid(), g.getContents()
        };
        ...
        if (jdbcTemplate.update(insertGallerSQL, params) > 0)
            isInsert = true;
    ...
    public List<BoardVO> selectBoard(int snum) {
        ...
        return jdbcTemplate.query(selectGallerSQL, mapper, params);
    }
    ...
    public BoardVO selectOneBoard(String bno) {
        ...
        BoardVO bvo = jdbcTemplate.queryForObject(
                                    selectOneGallerSQL, mapper, params);
